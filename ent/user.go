...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldName, user.FieldIntroduction, user.FieldEmail:
...
func (u *User) NamedBooks(name string) ([]*Book, error) {
	if u.Edges.namedBooks == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := u.Edges.namedBooks[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (u *User) appendNamedBooks(name string, edges ...*Book) {
	if u.Edges.namedBooks == nil {
		u.Edges.namedBooks = make(map[string][]*Book)
	}
	if len(edges) == 0 {
		u.Edges.namedBooks[name] = []*Book{}
	} else {
		u.Edges.namedBooks[name] = append(u.Edges.namedBooks[name], edges...)
	}
}
