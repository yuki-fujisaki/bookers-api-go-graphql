...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	for name, query := range uq.withNamedBooks {
		if err := uq.loadBooks(ctx, query, nodes,
			func(n *User) { n.appendNamedBooks(name) },
			func(n *User, e *Book) { n.appendNamedBooks(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadBooks(ctx context.Context, query *BookQuery, nodes []*User, init func(*User), assign func(*User, *Book)) error {
	...
	for _, n := range neighbors {
		fk := n.user_books
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_books" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_books" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) WithNamedBooks(name string, opts ...func(*BookQuery)) *UserQuery {
	query := (&BookClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if uq.withNamedBooks == nil {
		uq.withNamedBooks = make(map[string]*BookQuery)
	}
	uq.withNamedBooks[name] = query
	return uq
}
